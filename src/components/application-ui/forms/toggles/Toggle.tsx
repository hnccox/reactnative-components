...
const ToggleComponent = ({
	...
	if (optionLabels && !optionLabels.position) {
		optionLabels.position = 'inner';
	}
	...
	const [checkedLabelDimensions, setCheckedLabelDimensions] = useState({
		x: 0,
		y: 0,
		width: 0,
		height: 0,
	});
	const [uncheckedLabelDimensions, setUncheckedLabelDimensions] = useState({
		x: 0,
		y: 0,
		width: 0,
		height: 0,
	...
	const labelWidth = () => {
		...
		switch (size) {
			...
			case 'xl':
				...
				break;
			default:
				Number.isInteger(size) ? (_size = size as number) : (_size = 0);
		}
		if (optionLabels?.position === 'inner' || size === 'auto') {
			if (checkedLabelDimensions.width > uncheckedLabelDimensions.width) {
				return checkedLabelDimensions.width > _size
					? checkedLabelDimensions.width
					: _size;
			} else {
				return uncheckedLabelDimensions.width > _size
					? uncheckedLabelDimensions.width
					: _size;
			}
		} else {
			return _size;
	...
	const labelPosition = (portrait: boolean): Object => {
		if (portrait) {
			return label?.position && label?.position.split(' ')[0] === 'top'
				? { flexDirection: 'column' }
				: { flexDirection: 'column-reverse' };
		} else {
			return label?.position && label?.position.split(' ')[1] === 'left'
				? { flexDirection: 'row' }
	...
	React.useEffect(() => {
		Animated.timing(translateXAnim, {
			toValue:
				(isEnabled ? 1 : 0) *
				(toggleDimensions.width - sliderDimensions.width - 2 * 2),
			duration: 200,
			useNativeDriver: true,
		}).start();
	}, [isEnabled, toggleDimensions, sliderDimensions, translateXAnim]);
		...
		<>
			...
			{optionLabels && (
				<View
					style={[
						tw.style(
							'absolute inset-0 opacity-0 z-0 flex flex-row justify-start',
						),
					]}>
					<Text
						onLayout={event => {
			...
			<View
				style={[
					tw.style('flex items-center justify-between w-full'),
					labelPosition(isPortrait),
				]}>
				{/* Label */}
				...
				<View style={tw.style('flex flex-row items-center m-3 px-2')}>
								...
								style={[
									tw.style(
										...
										isDisabled
											...
											: 'text-slate-500',
										optionLabels.position === 'outer' ||
											(optionLabels.position === 'outer left' && !isEnabled)
											? 'opacity-100'
											: 'opacity-0',
									),
								]}>
					...
					<TouchableOpacity
						...
						style={[
							{
								minWidth: sliderWidth() * 2 + labelWidth(),
								width: sliderWidth() * 2 + labelWidth(),
								maxWidth: sliderWidth() * 2 + labelWidth(),
							},
							tw.style(
								isDisabled
									? isEnabled
									...
									? 'bg-indigo-600'
									: 'bg-gray-200',
								'relative flex flex-row h-6 rounded-full border-2 border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2',
							),
						]}
						disabled={isDisabled}>
						...
						<Animated.View
							...
							style={[
								tw.style('h-5 w-5 rounded-full bg-white shadow'),
							...
							{icons && !isEnabled && (
								<View
									style={[
										tw.style(
											'absolute inset-0 flex flex-col h-full w-full items-center justify-center',
											isEnabled ? 'opacity-0' : 'opacity-100',
										) /*{ opacity: opacityUncheckedAnim }*/,
									]}
									aria-hidden>
									<FontAwesomeIcon
										icon={['fas', `${icons.unchecked}`] as IconProp}
										size={12}
									/>
								</View>
							)}
							{icons && isEnabled && (
								<View
									style={[
										...
										) /*{ opacity: opacityCheckedAnim }*/,
									]}
									aria-hidden>
									<FontAwesomeIcon
										icon={['fas', `${icons.checked}`] as IconProp}
										size={12}
									/>
								</View>
							)}
						...
						{isEnabled && optionLabels?.position === 'inner' && (
							<Text
								style={[
									tw.style('absolute w-full self-center text-white pl-2'),
									{ textAlign: 'left' },
								]}>
								{optionLabels.checked}
							</Text>
						...
						{!isEnabled && optionLabels?.position === 'inner' && (
							<Text
								style={[
									tw.style('absolute w-full self-center text-slate-500 pr-2'),
									{ textAlign: 'right' },
								]}>
								{optionLabels.unchecked}
							</Text>
								...
								style={[
									tw.style(
										...
										isDisabled
											...
											: 'text-slate-500',
										optionLabels.position === 'outer' ||
											(optionLabels.position === 'outer right' && isEnabled)
											? 'opacity-100'
											: 'opacity-0',
									),
								]}>
